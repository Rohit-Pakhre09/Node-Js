...
const Todo = () => {
  ...
  return (
    <section
      className={`w-80 sm:w-sm md:w-md lg:w-lg rounded-xl ${light ? "bg-amber-200 shadow-white shadow-sm" : "bg-blue-200 shadow-lg"
        } h-150 md:h-200 p-5 transition-all duration-200 ease-in-out`}
    >
      {/* Title & Theme Toggle */}
      <section className="flex items-center justify-between">
        <p
          className={`font-bold text-4xl ${!light ? "text-black" : "text-neutral-600"
        ...
        <div>
          <button
            onClick={toggleTheme}
            className={`p-2 flex items-center justify-center ${light ? "bg-amber-500 shadow-lg" : "bg-white shadow-lg"
      ...
      {/* Headline */}
      <p
        className={`italic text-sm mt-4 ${!light ? "text-neutral-700" : "text-neutral-600"
          } transition-all duration-200 ease-in-out`}
      >
      ...
      <section className="flex items-center gap-5">
        ...
        <input
          ...
          className={`border border-neutral-500 mt-5 p-2 rounded-md outline-0 ${light
        ...
        <button
          onClick={handleAdd}
          className={`flex items-center justify-center mt-5 h-12 w-12 rounded-full p-2 cursor-pointer font-bold text-[20px] ${light
      ...
      <section>
            ...
            <ul className="space-y-4 list-none m-0 p-0">
              {[...todos]
                ...
                .map((todo) => (
                  <li
                    key={todo.id}
                    className={`p-4 flex items-center gap-5 rounded-md transition-all duration-500 ease-in-out ${light
                    ...
                    >
                      {editingId === todo.id ? (
                        <input
                          ...
                          className={`border px-2 py-1 rounded text-black w-23 sm:w-40 md:w-50 lg:w-65 placeholder:text-sm ${light
                        ...
                        <p
                          className={`truncate max-w-[65px] sm-max-w-[50px] md:max-w-[150px] lg:max-w-[200px]  transition-all duration-500 ${todo.status ? "line-through text-gray-400" : ""
                            }`}
                        >
                          {todo.title}
                        </p>
        ...
        {/* View Modal */}
        <section
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 ${view ? "block visible" : "hidden invisible"
            }`}
        >
